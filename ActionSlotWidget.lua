...
-- Modified version for use in ABR, original version at http://www.wowace.com/addons/ace-gui-3-0-action-slot-widgets/
...
do
	...
	local function Button_OnEnter(this)
		...
		if hasAction and not self:AcceptAction(actionType, actionData) then
	...
	local function Button_OnClick(this, button)
		...
		if (self.actionType or self.actionData) and not GetCursorInfo() then
			self:Fire("OnEnterPressed", "")
		else
			local hasAction, actionType, actionData
			if GetCursorInfo() then
				hasAction, actionType, actionData = ParseCursorInfo()
			else
				hasAction, actionType, actionData = self:AutoFill()
			end
			if hasAction and actionType and actionData and self:AcceptAction(actionType, actionData) then
	...
	local function SetText(self, text)
		...
		if actionType and actionData and name and texture and self:AcceptAction(actionType, actionData) then
...
do
	...
	local function AcceptAction(self, actionType, actionData)
		if actionType ~= "item" then return false end
		
		local data = self:GetUserDataTable()
		local path = data and data.path
		local slotName = path and path[#path]
		if slotName then
			local inventoryID = GetInventorySlotInfo(slotName)
			local itemEquipLoc = select(9, GetItemInfo(actionData))
			if itemEquipLoc and invtype_locations[itemEquipLoc] then
				for _, slotID in ipairs(invtype_locations[itemEquipLoc]) do
					if slotID == inventoryID then return true end
				end
			end
			return false
		else
	...
	local function BuildValue(self, actionType, actionData)
		return actionData and strmatch(actionData, '(item:[0-9:]+)')
	end
	
	local function AutoFill(self)
		local path = self:GetUserDataTable().path
		local slotName = path[#path]
		if slotName then
			return true, "item", self:BuildValue("item", GetInventoryItemLink("player", GetInventorySlotInfo(slotName)))
		end
	end
	
	local function ParseValue(self, value)
		if value then
			local name, _, _, _,  _, _, _, _,  _, texture = GetItemInfo(value)
			return "item", value, name, texture		
		end
	end
	...
	local widgetType = "ABRActionSlotItem"
